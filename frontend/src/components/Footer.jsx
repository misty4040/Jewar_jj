...
const Footer = () => {
    ...
    const social = (settings && settings.social) || {};
                ...
                {reviewList.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 pb-12 md:pb-24 border-b border-gray-100">
                        {reviewList.slice(0, 4).map((review) => (
                            <div key={review._id} className="flex flex-col gap-4 text-center md:text-left items-center md:items-start">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-12 gap-y-16 md:gap-y-20 py-12 md:py-24">
                    <div>
                        ...
                        <div className="flex flex-col gap-6">
                            ...
                            {contact.phone && (
                                <a href={`tel:${contact.phone}`} className="flex items-center gap-4 text-sm text-gray-600 hover:text-gold transition-colors group">
                                    <Phone size={18} className="text-gray-400 group-hover:text-gold" />
                                    {contact.phone}
                                </a>
                            )}
                            {contact.email && (
                                <a href={`mailto:${contact.email}`} className="flex items-center gap-4 text-sm text-gray-600 hover:text-gold transition-colors group">
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="flex gap-8 mt-auto">
                            {social.facebook && (
                                <a href={social.facebook} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-[11px] font-black tracking-[0.3em] uppercase text-gray-400 hover:text-gold transition-colors">
                                    <Facebook size={18} /> Facebook
                                </a>
                            )}
                            {social.instagram && (
                                <a href={social.instagram} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-[11px] font-black tracking-[0.3em] uppercase text-gray-400 hover:text-gold transition-colors">
